...
var Mispelr = function(config) {

  if(!(this instanceof Mispelr)) {
    return new Mispelr(config);
  }

  var pos = require('pos'),
      lexer = new pos.Lexer(),
      maledict = require('../data/maledict'),
      homophones = require('../data/homophones');
  ...
  var logger = function(msg, priority) {
    if (config.log || priority) console.log(msg); // eslint-disable-line no-console
  };
  ...
  var randomRespell = function(word) {
    ...
    if (isAlpha(word)) {
      var found = (maledict[word.toLowerCase()] != undefined);
  ...
  var respell = function(phrase, method) {
    ...
    method = (method != undefined ? method : spelltype.random);
  ...
  this.random = function(text) { return respell(text, spelltype.random); };
  this.maledict = function(text) { return respell(text, spelltype.maledict); };
  this.awfowell = function(text) { return respell(text, spelltype.awfowell); };
  this.constonantSorrow = function(text) { return respell(text, spelltype.constonantSorrow); };
  this.maxmister = function(text) { return respell(text, spelltype.maxmister); };
  this.homophilia = function(text) { return respell(text, spelltype.homophilia); };

};

module.exports = Mispelr;
